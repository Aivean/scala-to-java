 ...
 *
 * This source code is based on Mono.Cecil from Jb Evain, Copyright (c) Jb
 * Evain;
...
/**
 * I had to patch this class in order to make it decompile class
 * name like '_$$anon$1$A$'.
 */
public class CoreMetadataFactory implements MetadataFactory {
    ...
    private CoreMetadataFactory(final TypeDefinition owner, final
    IMetadataResolver resolver, final IGenericContext scope) {
        _owner = owner;
        _resolver = resolver;
        _scope = scope;
    ...
    public static CoreMetadataFactory make(final TypeDefinition owner, final
    IGenericContext scope) {
        return new CoreMetadataFactory(VerifyArgument.notNull(owner, "owner")
                , owner.getResolver(), scope);
    }

    public static CoreMetadataFactory make(final IMetadataResolver resolver,
                                           final IGenericContext scope) {
    ...
    public GenericParameter makeTypeVariable(final String name, final
        ...
        if (superBound == null || superBound == BottomSignature.make()) {
            if (extendsBound == null ||
                    (extendsBound instanceof SimpleClassTypeSignature &&
                            StringUtilities.equals("java.lang.Object", (
                                    (SimpleClassTypeSignature) extendsBound)
                                    .getName()))) {
    ...
    public GenericParameter findTypeVariable(final String name) {
        for (int i = _tempScope.size() - 1; i >= 0; i--) {
            ...
            if (genericParameter != null && StringUtilities.equals
                    (genericParameter.getName(), name)) {
                return genericParameter;
            }
        }
    ...
    private InnerClassEntry findInnerClassEntry(final String name) {
        ...
        final SourceAttribute attribute = SourceAttribute.find(AttributeNames
                .InnerClasses, _owner.getSourceAttributes());

        if (attribute instanceof InnerClassesAttribute) {
            final List<InnerClassEntry> entries = ((InnerClassesAttribute)
                    attribute).getEntries();

            for (final InnerClassEntry entry : entries) {
                if (StringUtilities.equals(entry.getInnerClassName(),
                        internalName)) {
                    return entry;
                }
            }
    ...
    public TypeReference makeNamedType(final String name) {
        ...
        if (entry != null) {
            ...
            final String shortName = StringUtilities.isNullOrEmpty(entry
                    .getShortName()) ? null : entry.getShortName();
            final TypeReference declaringType;

            if (!StringUtilities.isNullOrEmpty(entry.getOuterClassName())) {
                declaringType = makeNamedType(entry.getOuterClassName()
                        .replace('/', '.'));
            } else {
                ...
                while (lastDollarIndex >= 1 &&
                    ...
                    if (lastDollarIndex > 1) {
                        lastDollarIndex = name.lastIndexOf(lastDollarIndex,
                                lastDollarIndex - 2);
                    } else {
                        lastDollarIndex = -1;
                    }
                ...
                if (lastDollarIndex != -1) {
                    declaringType = makeNamedType(name.substring(0,
                            lastDollarIndex).replace('/', '.'));
                } else {
                    return new UnresolvedType(
                            packageEnd < 0 ? innerClassName : innerClassName
                                    .substring(packageEnd + 1),
                            name, null);
                }
        ...
        return new UnresolvedType(
                packageEnd < 0 ? StringUtilities.EMPTY : name.substring(0,
                        packageEnd),
    ...
    private final static class ClassSignature implements IClassSignature {
        ...
        private ClassSignature(
            ...
            _interfaceTypes = VerifyArgument.noNullElements(interfaceTypes,
                    "interfaceTypes");
            _genericParameters = VerifyArgument.noNullElements
    ...
    private final static class MethodSignature implements IMethodSignature {
        ...
        MethodSignature(
            ...
            final ParameterDefinition[] parameters = new
                    ParameterDefinition[parameterTypes.size()];

            for (int i = 0, slot = 0, n = parameters.length; i < n; i++,
                    slot++) {
        ...
        @Override
        public GenericParameter findTypeVariable(final String name) {
            for (final GenericParameter genericParameter :
    ...
    private final class UnresolvedType extends TypeReference {
        ...
        UnresolvedType(final TypeReference declaringType, final String name,
            ...
            setDeclaringType(VerifyArgument.notNull(declaringType,
                    "declaringType"));
        ...
        UnresolvedType(final String packageName, final String name, final
        String shortName) {
        ...
        UnresolvedType(final TypeReference declaringType, final String name,
                       final String shortName, final List<GenericParameter>
                               genericParameters) {
            _name = VerifyArgument.notNull(name, "name");
            _shortName = shortName;
            setDeclaringType(VerifyArgument.notNull(declaringType,
        ...
        UnresolvedType(final String packageName, final String name, final
        String shortName, final List<GenericParameter> genericParameters) {
            _packageName = VerifyArgument.notNull(packageName, "packageName");
            _name = VerifyArgument.notNull(name, "name");
            _shortName = shortName;
        ...
        @Override
        public <R, P> R accept(final TypeMetadataVisitor<P, R> visitor, final
        P parameter) {
